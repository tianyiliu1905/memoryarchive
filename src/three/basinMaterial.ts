...
export const basinFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    /* ---- 边缘光圈 ----
       ...
       中心从 0.78 内收到 0.70，是为了给外侧的点圈（BasinRing）腾地方。
       盆全景下相机固定在 y=9.2 / fov 42，盆平面上的可见半高只有
       3.53 个世界单位，而 0.78 已经对应到 3.28——光圈之外只剩 0.25，
       一圈点根本放不下。内收后光圈落在 2.94，点带中心 3.05 正好
       落在它外侧。

       强度从 0.085 降到 0.035，这是点圈能否被看见的关键。

       0.085 时光圈峰值处的亮度是 1.0094——已经溢出并被截断成
       纯白。而点圈正落在这条光带上（uv 0.726 处亮度 1.0034），
       白点画在纯白上，无论多不透明，差值都是 0 灰阶，等于隐形。
       这正是之前「白色圆点没出现」的真正原因，不是没画，是画了
       也看不见。

       降到 0.035 后峰值回到 0.9753，白点能拿到约 5 灰阶的对比——
       淡，但确实存在。光圈本身仍是「若有若无的一层」，符合它
       原本的定位。 */
    float sheen = exp(-pow(r - 0.70, 2.0) * 26.0) * 0.035;
